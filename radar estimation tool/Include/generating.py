...
def radarCube_init(Params, dataSet, CFAR):
...
def processingChain_rangeFFT(Params, dataSet, frameIdx):
    """
    ...
    if CFAR is True:
        doppler_idx, range_idx = np.where(dB_Doppler_fft == 1)
    else:
        doppler_idx, range_idx = np.where(doppler_FFT[0] >= 35000)
...
def dp_updateFrameData(frameIdx, Params, dataSet):
    """
    ...
    if fidIdx < Params['numBinFiles']:
        ...
        # Perform rangeFFT        
        processingChain_rangeFFT(Params, dataSet, frameIdx)
